...
## Introduction
Technical documentation applies to all the documentation relative to the project — the README, the development guide, the game-design doc, and other MD docs that describe how the project is structured, runs, and operates. This is a frontend-only Phaser game: there is no data model or API spec; [game-design.md](./game-design.md) is the single source of truth for game rules and entities, replacing them.
AI specs refers to the documents that explain AI agents how to behave, document, plan, code, etc, which includes team agreements, standards and conventions (the `*-standards.md` files and the contents of `ai-specs/`).

### Documentation map (source of truth)

| Doc | Owns |
|---|---|
| [base-standards.md](./base-standards.md) | Core dev rules, language policy, OpenSpec/agent conventions |
| [frontend-standards.md](./frontend-standards.md) | Project structure, Phaser/Vite/Vitest conventions, git workflow |
| [game-design.md](./game-design.md) | Game rules, entities, scoring, lives, scenes (gameplay source of truth) |
| [development_guide.md](./development_guide.md) | Setup, run, build and test instructions |
| [openspec-tasks-mandatory-steps.md](./openspec-tasks-mandatory-steps.md) | Mandatory steps when authoring/implementing `tasks.md` |
...
2. Identify which documentation files need updates based on the changes. Some clear examples:
   - For game rule, entity, scoring or lives changes: update docs/game-design.md
   - For scene/event-contract or project-structure changes: update docs/frontend-standards.md (and docs/game-design.md if a scene responsibility changes)
   - For changes in libraries, build config, or anything that changes the setup/run process: update the relevant *-standards.md and docs/development_guide.md
