...
- Optional spec context from user:
  - Direct ticket id in text (for example: issue number like `#2`)
  - Feature name
  - Endpoint
  - Frontend route
...
1. Identify the target spec/change:
   ...
   - If user text contains a ticket id pattern like `#[0-9]+`, use it as primary context (example: `show me #2`).
