...
If the native flow does not propagate Claude settings and the user expects parity with the main checkout, copy `.claude/settings.json` and `.claude/settings.local.json` from the primary workspace using the same loop as Step 1b **after** the native tool finishes—only when files exist on disk.
...
After creating the worktree with git (Step 1b), copy local Claude settings from the **main checkout** so Claude CLI behavior matches the primary workspace. These files are often untracked.
