...
│   ├── frontend-standards.md    # Phaser/Vite conventions
│   ├── game-design.md           # Game rules, entities, scoring (replaces api-spec/data-model)
...
- **`CLAUDE.md`** → Optimized for Claude
...
- Include docs/frontend-standards.md, docs/game-design.md, docs/documentation-standards.md.
- Tell the agent to adopt ai-specs/agents/frontend-developer.md for implementation.
...
context: |
  ...
  Architecture: Phaser Scenes (Boot/Game/UI); pure game logic in src/systems/ (no Phaser deps), thin scenes
  We use conventional commits
  Domain: Breakout-style arcade game (paddle, ball, bricks)
  ...
  - docs/frontend-standards.md — Phaser/Vite conventions, project structure, testing
  - docs/game-design.md — game rules, entities, scoring (single source for gameplay)
  - docs/documentation-standards.md — docs structure and maintenance
  For implementation: adopt ai-specs/agents/frontend-developer.md. Use ai-specs/skills/ for workflow guidance when applicable.
...
rules:
  ...
  _global:
    ...
    - For implementation artifacts, read docs/frontend-standards.md and adopt guidelines from ai-specs/agents/frontend-developer.md
    - Use docs/game-design.md for gameplay consistency (entities, rules, scoring) in specs and tasks
```
...
- **Claude**: `CLAUDE.md` → `docs/base-standards.md`
...
This project anchors changes on **GitHub Issues** (not Jira). The `enrich-us` skill reads issues via the `gh` CLI (`gh issue view <n> --comments`) or the GitHub MCP server, and writes the enriched story back as a comment plus a `refined` label.

MCP integrations:

- **GitHub MCP** (optional; alternative to `gh` CLI in `/enrich-us`): read/comment on issues directly.
- **Playwright MCP** (E2E; configured in `.mcp.json`): browser checks for gameplay flows.
  Mandatory for user-facing changes, optional only for pure `src/systems/` logic —
  see [openspec-tasks-mandatory-steps.md](../docs/openspec-tasks-mandatory-steps.md).
...
```bash
/enrich-us 2
/ff 002-paddle-and-ball
/apply 002-paddle-and-ball
/verify 002-paddle-and-ball
/archive 002-paddle-and-ball
/commit
```
...
Skills live in `ai-specs/skills/` and are mirrored into `.claude/skills/` via relative symlinks, so the agent can discover them. The agent loads a skill automatically when a request matches its description (per `AGENTS.md` §4). The most useful ones in day-to-day work are **`enrich-us`**, **`using-git-worktrees`**, **`writing-skills`**, and **`code-auditing`**:

- **`enrich-us`** — Analyze and enhance a vague GitHub issue (or raw idea) into an implementation-ready ticket with acceptance criteria, technical detail, and edge cases. Use **before** planning to make sure the team and the AI agree on scope.
...
- **Frontend Standards**: `docs/frontend-standards.md`
  - Phaser scene/object patterns
  - Pure logic in src/systems/ (no Phaser deps)
...
5. **Keep the symlink structure**: Remember to create relative symlinks from `.claude` to the corresponding `ai-specs/agents` and `ai-specs/skills` entries to keep it consistent
...
- Update docs/game-design.md so it matches the real entities and rules of this project.
...
- **Canonical Placement**: Prefer `ai-specs` as canonical source and expose through symlinks for `.claude` compatibility
...
- **Game Design**: `docs/game-design.md` (entities, rules, scoring)
  - *Single source of truth for gameplay; replaces backend api-spec/data-model*
- **Development Guide**: `docs/development_guide.md` (Setup, workflows)
  - *Setup instructions for the Vite/Phaser stack*
